...
/**
 * Abstract base class for Klondike game variants.
 * Contains common functionality shared by all variants.
 */
public abstract class AKlondike implements KlondikeModel<KlondikeCard> {
    protected List<Pile> cascadePiles;
    protected List<Pile> foundationPiles;
    protected List<KlondikeCard> drawPile;
    protected int numDraw;
    protected boolean gameStarted;

    /**
     * Creates a new Klondike game.
     */
    public AKlondike() {
    ...
    @Override
    public void startGame(List<KlondikeCard> deck, boolean shuffle,
        ...
        setupInitialVisibility(numPiles);
    ...
    /**
     * Sets up initial card visibility for cascade piles.
     * Different variants may have different rules.
     * @param numPiles number of cascade piles
     */
    protected abstract void setupInitialVisibility(int numPiles);

    /**
     * Checks if a card can be moved onto another card in cascade.
     * @param cardToMove the card being moved
     * @param targetCard the card to move onto
     * @return true if valid move
     */
    protected abstract boolean isValidCascadeMove(KlondikeCard cardToMove,
                                                  KlondikeCard targetCard);

    /**
     * Checks if cards being moved together form a valid build.
     * @param cards the cards to check
     * @return true if valid build for moving
     */
    protected abstract boolean isValidBuildToMove(List<KlondikeCard> cards);

    /**
     * Checks if any card can be moved to empty cascade pile.
     * @param card the card to check
     * @return true if this card can go in empty pile
     */
    protected abstract boolean canMoveToEmptyPile(KlondikeCard card);
    ...
    public void movePile(int srcPile, int numCards, int destPile) {
        ...
        for (int i = startIndex; i < source.size(); i++) {
            cardsToMove.add(source.getCard(i));
        }

        if (!isValidBuildToMove(cardsToMove)) {
            throw new IllegalArgumentException("Invalid build to move");
        }

        KlondikeCard topCard = cardsToMove.get(0);

        if (dest.isEmpty()) {
            if (!canMoveToEmptyPile(topCard)) {
                throw new IllegalArgumentException("Cannot move to empty pile");
            }
        } else {
    ...
    public void moveDraw(int destPile) {
        ...
        if (dest.isEmpty()) {
            if (!canMoveToEmptyPile(card)) {
                throw new IllegalArgumentException("Cannot move to empty");
            }
        } else {
            KlondikeCard destBottom = dest.getLastCard();
    ...
    protected boolean isValidFoundationMove(KlondikeCard card, Pile foundation) {
            ...
            if (!pile.isEmpty()) {
                ...
                for (int j = 0; j < cascadePiles.size(); j++) {
                    if (i != j) {
                        ...
                        if (dest.isEmpty() && canMoveToEmptyPile(card)) {
                            return true;
    ...
    protected void validateDeck(List<KlondikeCard> deck) {
    ...
    protected int countAces(List<KlondikeCard> deck) {
        int count = 0;
        for (KlondikeCard card : deck) {
